  ...
  context("Invalid Data", () => {
    it("does not allow `name` attribute to be empty", async () => {
      try {
        ...
        try {
          const insect = Insects.build({ millimeters: 5 });
    ...
    it("does not allow `millimeters` attribute to be empty", async () => {
      try {
        ...
        try {
          const insect = Insects.build({ name: "Wasp" });
          await insect.validate();
        } catch (err) {
          return;
        }
  ...
  context("Valid Data", () => {
    ...
    it("allows a string for the `description` attribute", async () => {
      await Insects.create({
        name: "Beetle",
        millimeters: 5,
        description: "A buggy beetle",
      });
    });

    it("uses the current time as default values for `createdAt` and `updatedAt` attribute values", async () => {
      await runSQL(
        "INSERT INTO 'Insects' (name, millimeters) VALUES ('Ladybug', 5);"
      );
      const testInsect = await Insects.findOne({
        where: { name: "Ladybug" },
        attributes: ["createdAt", "updatedAt"],
      });
      expect(testInsect.get("createdAt")).to.be.an.instanceof(Date);
      expect(testInsect.get("createdAt").toString()).to.eq(
        testInsect.get("updatedAt").toString()
      );
    });
  });
});
